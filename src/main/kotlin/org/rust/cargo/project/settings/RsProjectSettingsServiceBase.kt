...
import org.rust.cargo.project.settings.RsProjectSettingsServiceBase.StateBase
import kotlin.reflect.KProperty1
import kotlin.reflect.full.findAnnotation
import kotlin.reflect.full.memberProperties

abstract class RsProjectSettingsServiceBase<T : StateBase<T>>(val project: Project, state: T) : SimplePersistentStateComponent<T>(state) {

    abstract class StateBase<T : StateBase<T>> : BaseState() {
        abstract fun copy(): T
    }
    ...
    interface RsSettingsListener {
        fun <T : StateBase<T>> settingsChanged(e: SettingsChangedEventBase<T>)
    }
    ...
    abstract class SettingsChangedEventBase<T : StateBase<T>>(val oldState: T, val newState: T) {
